...
// Canvas UI "Clouds" component (WebGL2 shader fog), adapted for this project.
// Source: vendor "Canvas UI: Clouds (React)" component, copied in verbatim
// except for one deliberate change — see the `overflow` note below.
//
// Deviation from vendor source: both content-wrapper divs (native
// html-in-canvas branch and the DOM fallback branch) hardcoded
// `overflow: "auto"`. This project uses this component to wrap About's
// "Pre-About" block, whose two mountain illustrations intentionally bleed
// past that block's own bottom edge (see About.tsx's z-10 stacking comment).
// `overflow: auto` would silently clip that bleed, so both instances are
// changed to `overflow: "visible"` here.
...
export function createClouds(
  ...
  // Forced off: this browser's html-in-canvas support (drawElementImage +
  // requestPaint) was detected as present but never actually delivered real
  // pixels into `contentTexture`, which stays initialized to a fully opaque
  // BLACK 1x1 placeholder (see the texImage2D call below). COMPOSITE_FRAG's
  // uHasContent branch then blends that placeholder in — and its alpha
  // ramps up (via `aF`) faster than its color mix ratio (`cloudA`) — so the
  // fog rendered as solid black ink blots instead of tinted sand fog. This
  // project's config doesn't use refraction or fogBlur (the only features
  // that depend on captured content), so disabling this path costs nothing.
...
export function Clouds({
  ...
  // Forced off to match `htmlInCanvas = false` in createClouds() above — see
  // that comment for why. Keeping this in sync matters: if this stayed true
  // while the WebGL side ignores content capture, the source canvas would
  // render its native (visible) DOM structure while the shader never reads
  // from it, silently duplicating the content on screen.
  const native = false;
  useEffect(() => {
        ...
        {native ? (
          <div
            ...
            style={{
              ...
              width: "100%",
              height: "100%",
              // vendor default is "auto" — changed to "visible" so this
              // wrapper doesn't clip children that intentionally bleed past
              // its own box (About's mountain illustrations).
              overflow: "visible",
            }}
          >
            {children}
